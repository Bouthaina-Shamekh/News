...
@endphp
<div class="col-12 alert alert-danger" style="display: none;" id="alert">

</div>
<div class="row">
    <div class="form-group col-6 mb-3">
        <x-form.input name="title_org" label="{{ __('admin.Title') }}" :re="true" type="text"
            placeholder="{{ __('admin.enter news of title') }}" required :value="$news->title_org" />
    </div>
    <div class="form-group col-6 mb-3">
        <x-form.input name="date" label="{{ __('admin.Date') }}" placeholder="mm/dd/yyyy" :re="true"
            type="date" required :value="$news->date" />
    </div>
    <input type="hidden" name="visit" value="{{ $news->visit }}">
    <div class="form-group col-6 mb-3">
        <x-form.input name="keyword_org" class="TagifyBasic" label="{{ __('admin.enter keyword') }}" type="text"
            placeholder="{{ __('admin.enter keyword') }}" :value="$news->keyword_org" />
    </div>
    <div class="form-group col-6 mb-3">
        <label for="category_id" class="form-label">{{ __('admin.Categories') }}<span
    ...
    <div class="form-group col-6 mb-3">
        ...
        <input type="file" name="img_view" class="form-control" accept="image/*" @required($news->id == null) />
    ...
    <div class="form-group col-6 mb-3">
        ...
        @if ($news->vedio)
            <video src="{{ asset('storage/' . $news->vedio) }}" width="320" height="240"
    ...
    <script>
        ...
        tagifyElements.forEach(el => {
            const tagify = new Tagify(el, {
                // خيار يمنع تجاوز الحد الأقصى
                enforceMaxLength: true,
                // الحد الأقصى للطول الكلي (لكل الكلمات مجتمعة بما فيها الفواصل)
                maxLength: 255
            });

            // عرض تنبيه عند محاولة تجاوز الحد
            tagify.on('add', () => {
                const totalLength = tagify.value.map(item => item.value).join(',').length;
                if (totalLength > 255) {
                    tagify.removeTag(tagify.value[tagify.value.length - 1].value);
                    document.getElementById('alert').style.display = 'block';
                    document.getElementById('alert').innerHTML = 'الحد الأقصى للكلمات هو 255 حرفًا.';
                    setTimeout(() => {
