...
<x-site-layout>
            ...
            <div class="container podcasts-container">
                ...
                <section class="podcast-hero">
                    ...
                    <div class="hero-image-wrap" data-type="{{ $heroType }}" data-video-src="{{ $heroVideoSrc }}"
                        ...
                        <div class="hero-media-error"
                            style="display:none; position:absolute; inset:0; align-items:center; justify-content:center; padding:20px; text-align:center; color:#fff; background:rgba(0,0,0,.72); z-index:3;">
                        </div>
                            ...
                            <div style="flex: 1;">
                                ...
                                <div class="controls-row">
                                    ...
                                    <div class="share-icons">
                                        ...
                                        <!-- Share generic -->
                                        <a href="#" onclick="shareNative(event)" class="share-icon share-icon--gray" title="مشاركة">
                                            <svg viewBox="0 0 24 24" fill="white" width="15" height="15">
                                                <circle cx="18" cy="5" r="3" fill="none" stroke="white" stroke-width="2" />
                                                <circle cx="6" cy="12" r="3" fill="none" stroke="white" stroke-width="2" />
                                                <circle cx="18" cy="19" r="3" fill="none" stroke="white" stroke-width="2" />
    ...
    <script>
        $(document).ready(function() {
            ...
            const $heroMediaError = $heroWrap.find('.hero-media-error');
            const $waveform = $('.waveform');
            const mediaErrorText = @json(app()->getLocale() == 'ar' ? 'تعذر تشغيل هذه الحلقة. جرّب حلقة أخرى أو افتح الرابط مباشرة.' : 'This episode could not be played. Try another episode or open the link directly.');
            const missingMediaText = @json(app()->getLocale() == 'ar' ? 'لا يوجد مصدر وسائط لهذه الحلقة.' : 'This episode has no media source.');

            let mainAudio = null;
            let isMainPlaying = false;
            let isIframePlaying = false;
            let isClearingHeroVideo = false;
            const listAudios = {};
            ...
            if (heroVideoPlayer) {
                ...
                heroVideoPlayer.addEventListener('error', function() {
                    if (isClearingHeroVideo) return;
                    showMediaError(mediaErrorText, currentEpisode.videoSrc);
                });
            }
            ...
            function handlePlayAction() {
                ...
                if (!mainAudio) {
                    showMediaError(missingMediaText);
                    return;
                }
                ...
                if (isMainPlaying) {
                    ...
                    mainAudio.play().then(function() {
                        ...
                        isMainPlaying = false;
                        updateCurrentPlayButtons(false);
                        showMediaError(mediaErrorText, currentEpisode.audioSrc);
                    });
                    return;
                }
            ...
            function playHeroVideo() {
                ...
                if (!isDirectVideoUrl(videoUrl) && heroVideoIframe) {
                    if (heroVideoPlayer) {
                        heroVideoPlayer.pause();
                        heroVideoPlayer.removeAttribute('src');
                        heroVideoPlayer.load();
                        heroVideoPlayer.style.setProperty('display', 'none', 'important');
                    }

                    heroVideoIframe.src = embedUrl || videoUrl;
                ...
                isIframePlaying = false;
                heroVideoPlayer.style.setProperty('display', 'block', 'important');
                heroVideoPlayer.src = videoUrl;
                heroVideoPlayer.load();
                heroVideoPlayer.play().then(function() {
                    updateCurrentPlayButtons(true);
                }).catch(function() {
                    showMediaError(mediaErrorText, videoUrl);
                });
            }
            ...
            function initMainAudio(src) {
                ...
                mainAudio.addEventListener('error', function() {
                    isMainPlaying = false;
                    updateCurrentPlayButtons(false);
                    showMediaError(mediaErrorText, src);
                });
            }
            ...
            function stopHeroVideo() {
                ...
                if (heroVideoPlayer) {
                    isClearingHeroVideo = true;
                    heroVideoPlayer.pause();
                    heroVideoPlayer.removeAttribute('src');
                    heroVideoPlayer.load();
                    heroVideoPlayer.style.setProperty('display', 'none', 'important');
                    setTimeout(function() { isClearingHeroVideo = false; }, 0);
                }
            }

            function showMediaError(message, url) {
                stopHeroVideo();
                $heroWrap.addClass('is-video');
                $heroImg.hide();
                isMainPlaying = false;
                ...
                const link = url ? '<br><a href="' + escapeHtml(url) + '" target="_blank" rel="noopener" style="color:#fff;text-decoration:underline;">' + (document.documentElement.lang === 'ar' ? 'فتح الرابط' : 'Open link') + '</a>' : '';
                $heroMediaError.html('<div>' + escapeHtml(message) + link + '</div>').css('display', 'flex');
            }

            function hideMediaError() {
                $heroMediaError.hide().empty();
            }
