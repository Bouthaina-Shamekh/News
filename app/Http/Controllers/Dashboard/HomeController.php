...
class HomeController extends Controller
{
    ...
    public function createBackup()
    {
        ...
        try {
            ...
            Log::error('Database backup process could not run', [
                'message' => $exception->getMessage(),
                'mysqldump_path' => $mysqldumpPath,
            ]);
        }

        if ($this->createPhpDatabaseDump($backupFilePath, $database)) {
            return response()->download($backupFilePath)->deleteFileAfterSend(true);
        }
    ...
    private function createPhpDatabaseDump(string $backupFilePath, string $database): bool
    {
        try {
            ...
            fwrite($handle, '-- Created at: ' . Carbon::now()->toDateTimeString() . "\n\n");
            ...
            foreach ($this->databaseTables() as $table) {
                ...
                DB::table($table)->orderByRaw('1')->chunk(500, function ($rows) use ($handle, $table) {
                    foreach ($rows as $row) {
                        ...
                        $values = array_map(fn ($value) => $this->quoteSqlValue($value), array_values((array) $row));
